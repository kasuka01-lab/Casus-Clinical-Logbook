...
// Full report for a SINGLE patient/case, with photos.
export async function generateCaseReport(c, author, followUps = [], media = []) {
  try {
    if (!c) { alert("No case is open to report on."); return; }
    const mod = await import("jspdf");
    const jsPDF = mod.jsPDF || (mod.default && (mod.default.jsPDF || mod.default)) || mod;
    const doc = new jsPDF({ unit: "pt", format: "a4" });
    const W = doc.internal.pageSize.getWidth(), H = doc.internal.pageSize.getHeight();
    ...
    doc.setFont("times", "bold"); doc.setFontSize(22); doc.setTextColor(21, 41, 77);
    doc.text("Patient Report", M, y + 6); y += 16;
    doc.setDrawColor(192, 146, 46); doc.setLineWidth(2); doc.line(M, y, W - M, y); y += 18;
    doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(90, 104, 120);
    doc.text(`${c.case_no || ""}${c.date ? "  ·  " + c.date : ""}${author ? "  ·  " + author : ""}  ·  generated from Casus`, M, y); y += 22;

    const sectionHead = (label) => {
      need(24); doc.setFont("times", "bold"); doc.setFontSize(13); doc.setTextColor(21, 41, 77);
      doc.text(label, M, y); y += 6;
      doc.setDrawColor(220, 228, 242); doc.setLineWidth(0.7); doc.line(M, y, W - M, y); y += 14;
    };
    const para = (label, val) => {
      if (!val) return;
      if (label) { doc.setFont("helvetica", "bold"); doc.setFontSize(10); doc.setTextColor(156, 118, 32); need(15); doc.text(label, M, y); y += 13; }
      doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(30, 51, 84);
      const lines = doc.splitTextToSize(String(val), CW);
      for (const ln of lines) { need(14); doc.text(ln, M, y); y += 14; }
      y += 4;
    };
    const photo = async (m, maxW) => {
      const jp = await fetchJpeg(publicUrl(m.storage_path)); if (!jp) return;
      const dispW = Math.min(CW, maxW); const dispH = dispW * (jp.h / jp.w);
      need(dispH + (m.caption ? 18 : 10));
      try { doc.addImage(jp.data, "JPEG", M, y, dispW, dispH); } catch { return; }
      y += dispH + 3;
      const lbl = m.caption || (m.section && m.section !== "photo" ? m.section : "");
      if (lbl) { doc.setFont("helvetica", "italic"); doc.setFontSize(9); doc.setTextColor(90, 104, 120); doc.text(doc.splitTextToSize(lbl, dispW), M, y + 8); y += 14; }
      y += 8;
    };
    ...
    const demo = [
      ...
      c.age ? "Age: " + c.age : "",
      c.sex ? "Sex: " + c.sex : "",
      c.tribe ? "Tribe / ethnicity: " + c.tribe : "",
      c.hospital ? "Hospital / facility: " + c.hospital : "",
      c.patient_contact ? "Contact: " + c.patient_contact : "",
    ].filter(Boolean);
    para("", demo.length ? demo.join("\n") : "—");
    if (c.diagnosis) para("Diagnosis", c.diagnosis);
    if (c.specialty || c.category) para("Specialty", [c.specialty, c.category].filter(Boolean).join(" — "));
    if (c.involvement) para("Level of involvement", c.involvement);

    if (c.description) { sectionHead("History / presentation"); para("", c.description); }
    if (c.examination_findings) { sectionHead("Examination"); para("", c.examination_findings); }
    if (c.management) { sectionHead("Management"); para("", c.management); }
    if (c.discussion || c.challenges) { sectionHead("Discussion"); if (c.discussion) para("", c.discussion); if (c.challenges) para("Challenges", c.challenges); }
    if (c.outcome) { sectionHead("Outcome"); para("", c.outcome); }
    if (c.supervisor || c.supervisor_qualification) para("Supervised by", [c.supervisor, c.supervisor_qualification].filter(Boolean).join(" — "));

    // Follow-up & progress
    const fu = (followUps || []).slice().sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
    if (fu.length) {
      sectionHead("Follow-up & progress");
      for (const f of fu) {
        const tag = f.type === "progress" ? "Ward round" : "Follow-up";
        doc.setFont("helvetica", "bold"); doc.setFontSize(10); doc.setTextColor(156, 118, 32);
        need(14); doc.text(`${f.date || ""}  ·  ${tag}`, M, y); y += 13;
        if (f.note) { doc.setFont("helvetica", "normal"); doc.setFontSize(10.5); doc.setTextColor(30, 51, 84); for (const ln of doc.splitTextToSize(String(f.note), CW)) { need(14); doc.text(ln, M, y); y += 14; } }
        for (const m of (media || []).filter((mm) => mm.kind === "image" && mm.follow_up_id === f.id)) await photo(m, 220);
        y += 6;
      }
    }

    // Photos (main case images, excluding follow-up ones already shown)
    const imgs = (media || []).filter((m) => m.kind === "image" && !m.follow_up_id);
    if (imgs.length) { sectionHead("Photos & images"); for (const m of imgs) await photo(m, 320); }

    const safe = String(c.case_no || "patient").replace(/[^\w-]+/g, "-");
    doc.save(`casus-report-${safe}.pdf`);
  } catch (e) { console.error("case report error", e); alert("Sorry — the patient report couldn't be generated. " + (e && e.message ? e.message : "Please try again.")); }
}
