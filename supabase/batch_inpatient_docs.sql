-- ============================================================================
-- Casus — enable document uploads, admitted-patient daily progress, and the
-- next-review date. Matches the app code. Safe to run once (idempotent).
-- Run: Supabase -> SQL Editor -> paste -> Run.
-- ============================================================================

-- allow document/file uploads alongside images and videos
...
-- admission details + review reminder
...
-- dated notes: 'progress' (daily, while admitted) or 'followup' (after discharge)
alter table public.follow_ups add column if not exists type text not null default 'followup';

-- shared (examiner) view — include new fields; still hides patient identity
...
-- admin view — include new fields; still hides patient identity
