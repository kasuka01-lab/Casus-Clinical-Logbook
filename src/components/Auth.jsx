  ...
  const submit = async () => {
    ...
    try {
      if (tab === "signup") {
        ...
        const full_name = `${first_name} ${surnameValue}`;
        if (!first_name || !surnameValue) { setErr("Enter your first name and surname."); setBusy(false); return; }
        const { data, error } = await supabase.auth.signUp({
          email: email.trim(),
          password: pw,
          options: {
            emailRedirectTo: VERIFY_REDIRECT,
            data: { full_name, first_name, surname: surnameValue },
          },
        });
        ...
        if (!data?.user?.id) {
          setErr("Signup did not create a new user. Please try again or check your Supabase Auth settings.");
          setBusy(false); return;
        }
        if (Array.isArray(data.user.identities) && data.user.identities.length === 0) {
          setErr("Signup did not create a new user for this email. Try logging in or use Forgot password.");
          setBusy(false); return;
        }
        // Do NOT sign in — the user must confirm their email first.
        if (data.session) await supabase.auth.signOut();
        setMsg("Account created. Please check your email and verify your account before signing in.");
        setShowResend(true);
        setTab("login");
      } else {
